...
gboolean MyWebView::on_decide_policy(WebKitPolicyDecision *decision,
    ...
    // Scripted navigations must pass the same policy as user gestures.
    ...
    m_createParams.onNavigationRequest(g_lastNagivationRequestId, uri, isNewWindow);
    
    // navigation_action is borrowed from decision; WebKit releases it.
    return TRUE;
}

void MyWebView::allowNavigationDecision(int requestId, bool isAllowed) {
    auto entry = m_navigationRequestMap.find(requestId);
    if (entry == m_navigationRequestMap.end()) return;
    auto decision = entry->second;
    m_navigationRequestMap.erase(entry);
...
MyWebView::~MyWebView() {
    ...
    for (const auto& pending : m_navigationRequestMap) {
        webkit_policy_decision_ignore(pending.second);
        g_object_unref(pending.second);
    }
    m_navigationRequestMap.clear();
    gtk_widget_destroy(m_webview);
    g_object_unref(m_user_content_manager);
}
...
static void on_script_message_received(WebKitUserContentManager *manager,
    ...
    // The signal owns result; releasing it here corrupts the WebKit reference.
}
