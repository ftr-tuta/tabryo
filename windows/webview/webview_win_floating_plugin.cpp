...
void WebviewWinFloatingPlugin::HandleMethodCall(
  ...
  if (method_call.method_name() == "attachToView") {
    const auto value = arguments[flutter::EncodableValue("viewId")];
    const auto id = std::holds_alternative<int64_t>(value)
        ? std::get<int64_t>(value) : std::get<int32_t>(value);
    const auto target = m_registrar->GetViewById(id);
    if (!target || FAILED(webview->setParentWindow(target->GetNativeWindow()))) {
      result->Error("view_unavailable", "The destination window is unavailable.");
    } else {
      result->Success();
    }
